...
class hour_predictions:
    ...
    def predict(self, lst):
        ...
        try:
            ...
            data = pd.concat([season, mnth,hr, holiday, weekday, workingday, weathersit], axis=1)
            data.columns = ['season', 'mnth','hr', 'holiday', 'weekday', 'workingday', 'weathersit']
            ...
            data_new.columns = [f"season_{data_new['season'][0]}", f"mnth_{data_new['mnth'][0]}",f"hr_{data_new['hr'][0]}",
            ...
            cat = data_new.iloc[:, 0:7]
            num = data_new.iloc[:, 7:]

            lis1=['season_1', 'mnth_1','hr_0', 'holiday_0', 'weekday_0', 'workingday_0', 'weathersit_1']
